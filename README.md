# HlepMe
 # 从无到有 #APP开发需要哪些库 #自我总结
 
 ONE 界面：
 
 
 
一、 textview 
 是一个可以显示Html富文本的TextView。可以用于显示新闻、商品详情等场景。 XRichText  可直接用textview显示html网址https://github.com/limedroid/XRichText
 
 FolioReader-Android 完成度很高的一款 ePub Android 阅读器 
 https://github.com/FolioReader/FolioReader-Android
 
TagEditText，类似微博标签的文本控件 自定义包裹符号，如# ... # 或者 * ... *，或者 # 
https://github.com/limedroid/TagEditText

ticker一个滑动动画的textview（更好的显示温度，金额等） 
https://github.com/robinhood/ticker


一个textView的动画 
https://github.com/z56402344/AnimTextView


 
动态文字NumberAnimTextView   数字增加动画的 TextView
https://github.com/Bakumon/NumberAnimTextView

PinchZoomTextView 放大缩小的textview
https://github.com/androidessence/PinchZoomTextView

二、Edittext:
 登陆悬浮界面：利用新特性TextInputLayout布局
 http://www.jcodecraeer.com/a/basictutorial/2015/0821/3338.html 
 
 edittext字数输入限制，BufferTextInputLayout库 适用于留言反馈等内容。地址：
 https://github.com/bufferapp/BufferTextInputLayout
 
 丰富的edittext富文本编辑器
 https://github.com/chen-xiao-dong/RichEditText
 
 
 
三、 图片上传：
 用作个人用户显示几张照片 个性化照片介绍 可来回拖动  android-drag-square库 
 https://github.com/SwiftyWang/android-drag-square
 
 
 
 四、视频：
 视频播放  附带视频变小可移动 DragVideo 
 https://github.com/hejunlin2013/DragVideo
 
 
 五、分享：
 ShareUtil 一个综合性的分享及登录工具库，支持微信分享，微博分享，QQ分享，QQ空间分享以及Android系统默认分享，支持微信登录，微博登录以及QQ登录并获取用户信息。  
 https://github.com/shaohui10086/ShareUtil
 
 
 六、Webview：
Webview 伸缩 Toolbar 效果  CollapsingToolbar-With-Webview   
https://github.com/RameshBhupathi/CollapsingToolbar-With-Webview

 七、下拉刷新
 TwinklingRefreshLayout 小而强大的刷新控件，自带顺滑的越界回弹，v1.04 版精心重构，完美发布  https://github.com/lcodecorex/TwinklingRefreshLayout
 
 Android版的BreakOutToRefresh来了(弹块块的下拉刷新)
 https://github.com/Hitomis/FunGameRefresh
 
 
 
 
 八、酷炫特效、动画
 
 BoomMenu 爆炸展开菜单的按钮，加入了更多的按钮和新特性。
 https://github.com/Nightonke/BoomMenu

注册登陆：
SignUpTransition很概念化设计的一个注册动画效果。
https://github.com/JeasonWong/SignUpTransition

Material-Design风格的动画注册登陆界面～够清爽～
https://github.com/Yellow5A5/Material-Design-Login

一组资源组成背景动画 登录注册
FancyBackground
https://github.com/tslamic/FancyBackground


十个让用户印象深刻的动画 
https://stories.uplabs.com/10-impressive-uses-of-motion-and-animations-in-material-design-ecf8b94df02d?gi=7a9ff2571af5

通过 AnimateVectoreDrawer 实现的一些动画效果，看看代码，有没有新灵感。 
https://github.com/ghuiii/AnimationWithoutCode

让你的 App 不再那么无聊和单调，气泡风格的过渡动画效果，酷酷的！（老外投稿） 
https://github.com/Cleveroad/BubbleAnimationLayout

TagCloudView android 3D球形效果 
https://github.com/misakuo/3dTagCloudAndroid

android动画中的插值器，配图解说！
https://my.oschina.net/banxi/blog/135633

一个能够让 View 执行漂亮的漂浮动画的库
https://github.com/UFreedom/FloatingView

一个有趣的二元球加载动画 (
https://github.com/dodola/MetaballLoading

将语音输入显示为可视化的波纹动画的库 voice-recording-visualizer
https://github.com/tyorikan/voice-recording-visualizer

Android material风格activity切换过渡平移动画效果
https://github.com/toddway/MaterialTransitions

漂亮动画效果的 Checkbox，洋气！很标准的一个自定义组件，同时又是 很不错的 Path, PathMeasure 的一个学习教材。 (
https://github.com/CoXier/CheckBox


FloatingActionButton动画效果 (
https://github.com/Adirockzz95/FABtransitions

 checkbox
 SmoothCheckBox带动画的CheckBox
 https://github.com/andyxialm/SmoothCheckBox
 
 

 日历和工作表 ：
 ScrollablePanel
 支持多个方向滑动的 View，类似 Calendar 里的 UI 效果 
 https://github.com/Kelin-Hong/ScrollablePanel
 导航栏：
 Synopsis 很有新意的一款 Android Bottom Tab 效果。 
 https://github.com/thelong1EU/SpaceTabLayout
 

 翻页库：
 PageFlip
 https://github.com/eschao/android-PageFlip
 
 速度指示器，UI 再改进一下会好看很多很多～
 https://github.com/anastr/SpeedView
 
 可以模拟烟花等效果的一个库 
 https://github.com/plattysoft/Leonids
 

 仿Cuto的加载动画
 https://github.com/andyxialm/CutoLoadingView
 
 Android 方块加载器动画，很有新意。
 https://github.com/yuweiguocn/SquareLoading
 
 一个全能的水波加载动画
 https://github.com/yuweiguocn/SquareLoading
 
 超漂亮的 Animation Scale 动画设置效果。
 https://github.com/nickbutcher/AnimatorDurationTile
 
 动画插值器的编辑器 (
 https://github.com/MartinRGB/RapidInterpolator
 
 Yalantis 放出的一个新作品：TodDoList，拥有非常平滑漂亮的动画，值得学习。 
 https://github.com/Yalantis/ToDoList
 
 用Canvas实现的简单粒子动画
 https://github.com/JeasonWong/Particle

 模拟自然动画的精髓——TimeInterpolator与TypeEvaluator ‘
 https://github.com/xuyisheng/NaturalAnim
 
 Android 创造更精致的动画效果库，非常实用 ’
 https://github.com/thunderrise/android-TNRAnimationHelper
 
 加载动画ImageView 
 http://gank.io/search?q=%E5%8A%A8%E7%94%BB
 
 转场：
 兼容5.0以下转场动画帮助类 
 https://github.com/ImmortalZ/TransitionHelper
  
 使用Transition FrameWork实现有意义的转场动画
 https://github.com/lgvalle/Material-Animations/
 
 一个带渐变层叠动画的左右滑动效果 
 https://github.com/xiepeijie/SwipeCardView
 
 LoadingDrawable 一些酷炫的加载动画， 可以与任何View配合使用，作为加载动画或者Progressbar, 此外很适合与RecyclerRefreshLayout 配合使用作为刷新的loading 动画
 https://github.com/dinuscxj/LoadingDrawable
 
 数字动画效果
 https://github.com/Armijn/NumberViewAnimation
 
 
 设置信纸的动画效果
 https://github.com/alexvasilkov/FoldableLayout
 
 三十种不同的缓动动画(给代码家丢肥皂可不能缓动) 
 https://github.com/cimi-chen/EaseInterpolator
 
 android的指纹动画
 https://github.com/mattprecious/swirl
 
 MCMHeaderAnimated列表与详情页切换过程增加动画的库 
 https://github.com/mathcarignani/MCMHeaderAnimated
 
 
 九、seekbar
 BubbleSeekBar 自定义气泡seekbar  https://github.com/woxingxiao/BubbleSeekBar
 
 十、Android Lollipop联系人之PinnedListView简单使用
 https://git.oschina.net/way/PinnedHeaderListView
 
 十一、播放器：
 视频播放：
 jjdxm_ijkplayer 基于ijkplayer简单的UI界面 当前项目是基于ijkplayer项目进行的播放器界面UI封装。 是一个适用于 Android 的 RTMP 直播推流 SDK，可高度定制化和二次开发
 https://github.com/jjdxmashl/jjdxm_ijkplayer/
 
 GiraffePlayer
 https://github.com/tcking/GiraffePlayer
 
 
 JieCaoVideoPlayer（android videoplayer videoview fullscreen节操精选家开源的视频播放器解决方案  ）
 https://github.com/lipangit/jiecaovideoplayer
 
 音乐播放：
 
 
 
 十二、system bar
 SystemBarTint 一个可改变系统System bar颜色的透明的库
 https://github.com/jgilfelt/SystemBarTint
 
 十三、图片照片
 chrisbanes 大神的PhotoView 库，支持缩放和一系列手势
 https://github.com/chrisbanes/PhotoView
 
 crop-image-layout 图片裁剪
 https://github.com/yulu/crop-image-layout
 
 android-crop一个方便剪裁图片的库
 https://github.com/jdamcd/android-crop
 
 
 
 十四、下载
 FileDownloader Android 文件下载引擎，稳定、高效、灵活、简单易用
 https://github.com/lingochamp/FileDownloader/blob/master/README-zh.md
 
  
 GADownloading 蛮有创意的一发 Android 下载效果 
 https://github.com/Ajian-studio/GADownloading
 
 一个展示下载进度的View，下载完成和失败的时候会有酷酷的动画. 
 https://github.com/dudu90/FreshDownloadView
 
 
 十五、引导界面：
 TourGuide https://github.com/worker8/TourGuide
 
 自动通过用户启动 App 的次数，弹出 Rating 页面引导用户评价。
 https://github.com/codemybrainsout/smart-app-rate
 
 
 雅虎新闻引导动画效果
 https://github.com/rahulrj/YahooNewsOnboarding
 
 
 十六、recycleView
 RecyclerViewWithHeaderNewPractice一种新的 Header View + RecyclerView 实现方式 
 https://github.com/drakeet/RecyclerViewWithHeaderNewPractice
 
 给RecyclerView添加header与footer效果
 https://github.com/tumblr/Bookends
 
 
 
 十七、Fragment:
 UiBlock轻量级代替Fragment的库 
 https://github.com/tianzhijiexian/UIBlock/
 
 
 十八、dialog
 
 swipe的dialog 
 https://github.com/kakajika/SwipeAwayDialog
 
 
 十九、偏好 preference
 
 使用方便的的 Material Design 风格 Preference UI，包含一个根据规范自制的新的 SimpleMenuPreference 
 https://github.com/RikkaW/MaterialPreference
 
 
 二十、布局
 一个简单但可以自定义切换动画的管理多状态的自定义layout 
 https://github.com/lufficc/StateLayout
 
 
 
 
 
 
 
 
 
 
 
 
 
 
 
 
 
 
 
 
 
 
 
 
 
 
 
 
 
 TWO：
 依赖注解
 
 
 通俗易懂的Dagger2入门教程,值得一看 http://www.jianshu.com/p/1d84ba23f4d2
 


THREE 
框架：

我眼中的Android架构 
http://blog.chengdazhi.com/index.php/150



从MVC框架看MVC架构的设计
http://kb.cnblogs.com/page/502983/



Anko框架让Android开发快速且容易 
http://www.finalshares.com/read-6928

一个Android MVP模式实例项目
https://github.com/richardradics/MVPAndroidBootstrap


Android App的设计架构：MVC,MVP,MVVM与架构经验谈
https://www.sdk.cn/news/2501

一种更清晰的Android架构
https://github.com/hehonghui/android-tech-frontier/tree/master/androidweekly/%E4%B8%80%E7%A7%8D%E6%9B%B4%E6%B8%85%E6%99%B0%E7%9A%84Android%E6%9E%B6%E6%9E%84


一种在android中实现MVP模式的新思路
https://github.com/hehonghui/android-tech-frontier/tree/master/androidweekly/%E4%B8%80%E7%A7%8D%E5%9C%A8android%E4%B8%AD%E5%AE%9E%E7%8E%B0MVP%E6%A8%A1%E5%BC%8F%E7%9A%84%E6%96%B0%E6%80%9D%E8%B7%AF

FORE 
二维码 ：




android-zxingLibrary几行代码快速集成二维码扫描功能
https://github.com/yipianfengye/android-zxingLibrary

FIVE 
线程 

Android并发编程-线程间通信的三种基本方式
http://blog.csdn.net/manoel/article/details/38964563

Android Service与Activity之间通信的几种方式
http://blog.csdn.net/xiaanming/article/details/9750689


Android中程序与Service交互的方式——交互方式
http://blog.csdn.net/yihongyuelan/article/details/7216188

Android的进程与线程
http://android.jobbole.com/80640/

Andorid Binder进程间通信总结
http://blog.csdn.net/jltxgcy/article/details/30993741




Android 线程的正确使用姿势
http://android.jobbole.com/82440/

SIX：
android优化 性能



Android性能优化典范 - 第5季 ：
http://hukai.me/android-performance-patterns-season-5/

10 条提升 Android 性能的建议 
https://realm.io/cn/news/droidcon-farber-improving-android-app-performance/



SEVEN:
网络访问


rx-mvvm-android
RxJava来进行Android databinding的一种方式 
https://github.com/ffgiraldez/rx-mvvm-android

Volley是否支持https
http://www.jiangwenrou.com/volley%E6%98%AF%E5%90%A6%E6%94%AF%E6%8C%81https.html

HTTP协议之状态码详解
http://v5browser.iteye.com/blog/1769789

Android中网络操作使用总结
http://blog.isming.me/2014/05/11/use-network-in-android/

Android Volley 完全解析系列
http://blog.csdn.net/guolin_blog/article/details/17482095

一个建立在RxJava架构Android实例项目
https://github.com/reark/reark




EIGHT:
成品APP

MD花瓣App 
https://github.com/LiCola/huabanDemo

干货App 
https://github.com/tripleCC/GanHuoCode

又一个 Gank.io 客户端。 
https://github.com/Assassinss/smile

Gank干货集中营Android客户端。
https://github.com/GcsSloop/Gank

GankMM -- 干货营
https://github.com/maning0303/GankMM
 
 gankio 客户端大全：
 https://github.com/ruijun/awesome-gank.io
 
 
高仿 爱范儿 
https://github.com/iCodeForever/ifanr

felony带 UI 的 PGP 工具，不需要各种教程教你如何用 PGP 了，一点即用
https://github.com/henryboldi/felony

非官方版“一个(ONE)”，一个比官方版更优秀的版本 
https://github.com/Uphie/ONE-Unofficial


Aisen微博是新浪微博的第三方客户端，UI遵循Material Design
https://github.com/wangdan/AisenWeiBo


豆瓣FM (OC实现) 
https://github.com/XVXVXXX/DoubanFM







NINE：
android机制与解答：
一个经典例子让你彻彻底底理解java回调机制
http://blog.csdn.net/xiaanming/article/details/8703708

java并发面试题
http://blog.csdn.net/geolo/article/details/8670900

Android四大基本组件介绍与生命周期
http://www.cnblogs.com/bravestarrhu/archive/2012/05/02/2479461.html


Android 事件分发机制中的事件处理
http://blog.csdn.net/dawanganban/article/details/19285977

Android事件分发机制完全解析，带你从源码的角度彻底理解(上)
http://blog.csdn.net/guolin_blog/article/details/9097463

Android事件分发机制完全解析，带你从源码的角度彻底理解(上)
http://blog.csdn.net/guolin_blog/article/details/9153747

Android View 事件分发机制 源码解析 （上）
http://blog.csdn.net/lmj623565791/article/details/38960443

 Android ViewGroup事件分发机制
http://blog.csdn.net/lmj623565791/article/details/39102591

 Andriod 从源码的角度详解View,ViewGroup的Touch事件的分发机制
http://blog.csdn.net/xiaanming/article/details/21696315

什么是ANR 如何避免它？
http://blog.csdn.net/Zengyangtech/article/details/6025671

Android 高效加载大图、多图解决方案，有效避免程序 OOM
http://blog.csdn.net/guolin_blog/article/details/9316683






TEN： 
Android 数据存储与数据解析


android解析XML总结（SAX、Pull、Dom三种方式）
http://www.cnblogs.com/JerryWang1991/archive/2012/02/24/2365507.html



ELEVEN： 
Android 进程间通信

Android aidl Binder框架浅析
http://blog.csdn.net/lmj623565791/article/details/38461079

Android AIDL实例解析
http://blog.csdn.net/bboyfeiyu/article/details/39003759

Android进程间传递复杂数据（AIDL)
http://blog.csdn.net/dawanganban/article/details/17524581

Android IPC进程间通讯机制学习笔记
http://www.cnblogs.com/bluestorm/archive/2011/11/05/2298125.html








TWELVE 工具 插件等
android性能优化视频、文档和工具收集 
https://github.com/Juude/awesome-android-performance

快速生成selector 
https://github.com/inmite/android-selector-chapek


可以读取SD卡，asset文件夹，或者网络下载的pdf文件的widget 
https://github.com/voghDev/PdfViewPager

字体文件压缩神器 
https://github.com/forJrking/FontZip


绘制允许任何对SVG图形复杂的变形动画工具 
https://github.com/bonnyfone/vectalign#vectalign----

Android 矢量图动画在线辅助工具 
https://github.com/romannurik/AndroidIconAnimator


仿三星桌面的浮动工具
https://github.com/k0shk0sh/FastAccess

















